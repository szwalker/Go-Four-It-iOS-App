...
//  ArGame
//
//  Created by Chenyu Liu on 4/16/19.
//  Copyright © 2019 Chenyu Liu and Jiaqi Liu. All rights reserved.
//
...
l
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let touch = touches.first{
            ...
            if let hitResult = results.first{
                if hitResult.node.name! != "ball" && hitResult.node.name! != "text" && hitResult.node.name! != "locked"{
    ...
    // when the confirm button has been pressed
    ...
    func createBall(color: String, atposition: SCNVector3 ){
        ...
        node.geometry = ball;
        print("node position ", atposition)
        sceneView.scene.rootNode.addChildNode(node)
